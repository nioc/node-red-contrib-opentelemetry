...
async function parseAttribute (data, nodeType) {
  try {
    // @TODO handle the conf
    const confMap = {
      'home_assistant.action.name': '$.payload.action',
      'home_assistant.action.targets': '$.payload.targets.area_id',
    }
    const attributes = {}
    for (const key in confMap) {
    // eslint-disable-next-line security/detect-object-injection
      const expression = jsonata(confMap[key])
...
async function createSpan (tracer, msg, nodeDefinition, node, isNotTraced) {
  try {
    ...
    // create child span
    const localAttributes = await parseAttribute(msg, nodeDefinition.type)
...
module.exports = function (RED) {
  ...
  function OpenTelemetryNode (config) {
    ...
    // get config
    const { url, protocol, serviceName, rootPrefix, ignoredTypes, propagateHeadersTypes, isLogging, timeout } = config
    ...
    RED.hooks.add('postDeliver.otel', async (sendEvent) => {
      logEvent(node, '4.postDeliver', sendEvent)
      const span = await createSpan(tracer, sendEvent.msg, sendEvent.destination.node, node, ignoredTypesList.includes(sendEvent.destination.node.type))
