...
function parseAttribute (data, flowId, nodeType) {
  if (_attributeMappings.length === 0) {
    return
  }
  const attributes = {}
  _attributeMappings
    .filter((mapping) => (mapping.flow === '' || mapping.flow === flowId) && (mapping.nodeType === '' || mapping.nodeType === nodeType))
    .forEach((mapping) => {
      try {
...
function createSpan (tracer, msg, nodeDefinition, node, isNotTraced) {
  try {
    ...
    // create child span
    const localAttributes = parseAttribute(msg, nodeDefinition.z, nodeDefinition.type)
    if (_isLogging) {
      console.log(`Local span attributes: ${JSON.stringify(localAttributes)}`)
    }
